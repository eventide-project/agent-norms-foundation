# `agent/design/` — design docs that settle direction

`agent/design/` holds **design docs**: the settled *direction* of a piece of work, written before a plan sequences the build. A design decides what the shape should be — structure, construction, API, the model — and records the resolutions that later work rests on.

- **Relationship to plans:** a design settles direction; an `agent/plans/` plan **realizes** it. The design is the stable reference the plan points back to (its **Source design**).
...
- **Format:** the same frontmatter-free markdown as rules; the *document shape* (its sections) is the `docs` package's design-document-format rule. The filename follows the ISO-8601-UTC prefix convention (see the agent-file-names rule).
